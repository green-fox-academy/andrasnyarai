...
test('maxOfThree: max of three third', function (t) {
  t.equal(extend.maxOfThree(3, 8, 10), 10);
  t.end();
});

test('maxOfThree: max of from two', function (t) {
  t.equal(extend.maxOfThree(3, 10, 10), 10);
  t.end();
});
...
test('median: add string returns error message', function (t) {
  t.throws(function() { extend.median([1, 2, 3, 4, 'string'])}, 'Invalid value');
  t.end();
});

test('isVovel: is vovel a', function (t) {
  t.equal(extend.isVovel('a'), true);
  t.end();
});

test('isVovel: is vovel b', function (t) {
  t.equal(extend.isVovel('b'), false);
  t.end();
});

test('isVovel: empty string returns error message', function (t) {
  t.throws(function() { extend.isVovel('')}, 'Please, give me one character');
  t.end();
});

test('isVovel: more the one character returns error message', function (t) {
  t.throws(function () { extend.isVovel('life is better with bacon')}, 'Please, give me one character');
  t.end();
});
...
test('translate number returns error message', function (t) {
  t.throws(function () { extend.translate(12345)}, 'Invalid value');
  t.end();
});